...
export default function PostCard({ post }) {
  return (
    <div className="card p-4 transition-all duration-200 hover:shadow-card-hover hover:-translate-y-0.5 flex flex-col h-full w-full">
      <span className={`badge mb-1.5 ${post.category && post.category !== '其他' ? '' : 'invisible'}`} style={{ alignSelf: 'flex-start' }}>
        {post.category || '其他'}
      </span>
      <h3 className="font-display font-semibold line-clamp-2 mb-1.5" style={{ color: 'var(--color-text-primary)', fontSize: '1.0625rem' }}>
        {post.title}
      </h3>
      <p className="text-sm mb-3 flex-1" style={{ color: 'var(--color-text-body)' }}>
        {post.summary}
      </p>
